...
export type PartialAtom<T> =
    T | (
    T extends Array<infer U> ? PartialAtom<U>[] :
    T extends Map<infer K, infer V> ? Map<PartialAtom<K>, PartialAtom<V>> :
    T extends Set<infer M> ? Set<PartialAtom<M>> :
    T extends object ? {
        [K in keyof T]?: (
            T[K] extends BaseEntity ? number :
            T[K] extends BaseEntity | undefined ? number | undefined :
            T[K] extends BaseEntity[] ? number[] :
            T[K] extends BaseEntity[] | undefined ? number[] | undefined :
            PartialAtom<T[K]>)
    } : T);
...
export class BasicCrud<Entity extends Atom> {
    ...
    async readOneByOrFail(options: FindOptionsWhere<Entity>): Promise<Entity> {
        return await this.entity.findOneByOrFail<Entity>(options);
    };
